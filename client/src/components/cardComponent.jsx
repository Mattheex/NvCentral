  ...
  if (section === "Submit Data") {
    ...
    if (typeof info === "object" && !Array.isArray(info)) {
      ...
      field = (
        <>
          {info.map((item, index) => (
            <Card.Link key={index} href={item.link} target="_blank">
              {item.text}
            </Card.Link>
          ))}
        </>
  ...
  useEffect(() => {
    setInputAdd((prevInputAdd) => {
      ...
      if (info["Stage"][0] !== undefined) {
        updatedItems["Phenotype"][index] = {
          stage: info["Stage"][0].node,
          phenotype: info["Phenotype"][0].node,
          value: "",
        };
      }
      return updatedItems;
...
function CardComponent({ info, header, section, inputAdd, setInputAdd }) {
  ...
  if (header === "Phenotype") {
    body.push(
      <DLRow key={body.length} info={info["Other"]} section={section} setInputAdd={setInputAdd}></DLRow>
    );
    if (section === "Submit Data") {
      body.push(
        <Fragment key={body.length}>
          ...
          <Button
            className="shadow"
            variant="primary"
            onClick={() => setInputAdd({ ...inputAdd, Phenotype: inputAdd.Phenotype.concat({}) })}
          >
            +
          </Button>
        </Fragment>
      ...
      body.push(
        <Container key={body.length} className={"p-0 m-0"}>
