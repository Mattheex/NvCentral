        ...
        <ListGroup className="shadow rounded position-absolute z-2 mt-5" variant="flush">
            {(result.map((item, key) => (
                <ListGroup.Item key={key} action
                                as={Link} to={window.origin + '/' + routing[item.field] + '/' + item.id}>{item.label}</ListGroup.Item>
            )))}
        </ListGroup>
    )
}
...
function Home({routing}) {
    ...
    const Search = (field, value) => {
        ...
        console.log(value)
        if (value.length > 0) {
            fetch(`/search/${value}`).then(res => res.json().then(setResult))
        } else {
            setResult([])
        }
        ...
        <div className="d-flex flex-column flex-grow-1 align-items-center justify-content-center tableColor home">
            <div className="d-flex flex-column w-50 h-50 shadow-lg p-5 justify-content-between">
                <Form className="d-flex">
                    <Label className="me-2 shadow-lg" placeholder="MyHC1" type="search" handleChange={Search}
                           value = {inputValue}
                           handleBlur={handleBlur}/>
                    {result.length > 0 &&
                        <SearchResultList result={result} handleChange={handleChange} routing={routing}/>}
                    <Button ref={inputRef} variant="success" as={Link} to={`/all/${inputValue}`}>Search</Button>
                </Form>
                <div>
                    <Label className={"shadow-lg"} rows={3} placeholder={"BLAST sequence"} as={"textarea"}/>
                    <Button className="mt-2 shadow-lg" variant="success">Search</Button>
                </div>
            </div>
        </div>
    );
